...
/*   Updated: 2025/11/27 20:03:44 by mozinedd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	parse_color(char *line, t_map *map, uint32_t r)
{
	uint32_t	g;
	uint32_t	b;
	uint32_t	alpha;
	...
	if (!rgb || !rgb[0] || !rgb[1] || !rgb[2] || char_count(line, ',') > 2)
		return (free_arr(rgb),
			free_arr(splited_str), printf("Error\ninvalid format\n"), 0);
	r = valid_num(rgb[0]);
	g = valid_num(rgb[1]);
	b = valid_num(rgb[2]);
	...
	return (free_arr(rgb), free_arr(splited_str), 1);
}
